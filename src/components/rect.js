...
export const diffRect = (
  refA,
  refB,
  { alignments = objValues(ALIGNMENT_PROPS), absDistance = true } = {}
) => {
  ...
  const results = objMap(
    objReduce(
      ALIGNMENT_PROPS,
      (results, prop) => {
        return alignments.includes(prop)
          ? { ...results, [prop]: NaN }
          : results;
      },
      {}
    ...
    (_, prop) => {
      switch (prop) {
        case ALIGNMENT_PROPS.tt:
          return calc(rectA.top - rectB.top);
        case ALIGNMENT_PROPS.bb:
          return calc(rectB.bottom - rectA.bottom);
        case ALIGNMENT_PROPS.rr:
          return calc(rectB.right - rectA.right);
        case ALIGNMENT_PROPS.ll:
          return calc(rectA.left - rectB.left);
        case ALIGNMENT_PROPS.tb:
          return calc(rectA.top - rectB.bottom);
        case ALIGNMENT_PROPS.bt:
          return calc(rectB.top - rectA.bottom);
        case ALIGNMENT_PROPS.rl:
          return calc(rectB.left - rectA.right);
        case ALIGNMENT_PROPS.lr:
          return calc(rectA.left - rectB.right);
        case ALIGNMENT_PROPS.xx:
          return calc(
            (rectA.right - rectB.right + (rectA.left - rectB.left)) / 2
          );
        case ALIGNMENT_PROPS.yy:
          return calc(
            (rectA.top - rectB.top + (rectA.bottom - rectB.bottom)) / 2
          );
      }
    }
  );
...
Rect.prototype.diff = function(target, ...args) {
  return diffRect(this, target, ...args);
};
